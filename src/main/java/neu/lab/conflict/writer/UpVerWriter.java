	...
	public void write(String outPath,boolean append) {
		try {
			...
			for (NodeConflict nodeConflict : NodeConflicts.i().getConflicts()) {
				MavenUtil.i().getLog().info(nodeConflict.toString());
			...
			MavenUtil.i().getLog().info("result was written to "+outPath);
			printer.close();
		} catch (Exception e) {
...
//		MavenUtil.i().getLog().info("path1:"+path1);
//		MavenUtil.i().getLog().info("path2:"+path2);
...
//		MavenUtil.i().getLog().info("shortDep:"+shortDep);
//		MavenUtil.i().getLog().info("longDep:"+longDep);
...
//						MavenUtil.i().getLog().info("shortVer:"+num1);
						num2 = Integer.parseInt(longVersion[i]);
//						MavenUtil.i().getLog().info("shortVer:"+num2);
						if (num1 < num2) {
							return true;
						}
						if (num1 > num2) {
